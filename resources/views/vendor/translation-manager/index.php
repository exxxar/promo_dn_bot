...
<div class="container-fluid">
    ...
    <div class="alert alert-success success-import" style="display:none;">
        <p>
            Завершен импорт обработанных <strong class="counter"> N </strong> элементов! Перезагрузите страницу, чтобы
            обновить группы!</p>
    </div>
    <div class="alert alert-success success-find" style="display:none;">
        <p>
            Закончен поиск переводов, найдено <strong class="counter"> N </strong> товаров!</p>
    </div>
    <div class="alert alert-success success-publish" style="display:none;">
        <p>
            Закончена публикация переводов для группы '<?php echo $group ?>'!</p>
    </div>
    <div class="alert alert-success success-publish-all" style="display:none;">
        <p>Завершена публикация переводов для всей группы!</p>
    </div>
            ...
            <div class="row">
                <div class="col-sm-3">
                    <select name="replace" class="form-control">
                        <option value="0">Добавить новый перевод</option>
                        <option value="1">Изменить текущий перевод</option>
                    </select>
                </div>
                <div class="col-sm-2">
    ...
    <form class="form-find" method="POST"
        ...
        <div class="form-group">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <button type="submit" class="btn btn-info" data-disable-with="Searching..">Найти перевод в файле</button>
        </div>
    </form>
<?php endif; ?>
    <?php if (isset($group)) : ?>
    ...
    <form role="form" method="POST"
        ...
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
            <p>Выберите группу для отображения групповых переводов. Если группы не видны, убедитесь, что вы запустили
                миграции и импортировали переводы.</p>
        ...
        <div class="form-group">
            <input type="submit" class="btn btn-default" name="add-group" value="Добавить и редактировать ключи"/>
        </div>
    </form>
    <?php if ($group): ?>
        <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>"
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" rows="3" name="keys"
                          placeholder="Добавить 1 ключ на строку без префикса группы"></textarea>
            </div>
            <div class="form-group">
                    ...
                    <?php if ($deleteEnabled): ?>
                        <td>
                            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@postDelete', [$group, $key]) ?>"
                               class="delete-key"
                               data-confirm="Вы уверены, что хотите удалить переводы для '<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>?"><span
    ...
    <?php else: ?>
            ...
            <form class="form-add-locale" method="POST" role="form"
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <button type="submit" class="btn btn-default btn-block" data-disable-with="Добавление...">
        ...
        <fieldset>
            ...
            <form class="form-inline form-publish-all" method="POST"
                  ...
                  data-remote="true" role="form"
                  data-confirm="Вы уверены, что хотите опубликовать всю группу переводов? Это перезапишет существующие языковые файлы.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-primary" data-disable-with="Публикуется...">Публиковать всё
                </button>
            </form>
        </fieldset>
